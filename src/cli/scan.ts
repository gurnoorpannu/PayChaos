...
function printHumanReport(result: Awaited<ReturnType<typeof scanRepository>>) {
  ...
  for (const surface of result.webhookSurfaces) {
    ...
    console.log(
      `  controls     signature=${surface.signatureVerification} event-id=${surface.eventIdIdempotency} atomic-claim=${surface.atomicEventClaim} transaction=${surface.transactionBoundary} monotonic=${surface.monotonicStateGuard} outbox=${surface.durableOutbox}`
    );
  }
